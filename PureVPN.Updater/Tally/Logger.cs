...
namespace Tally
{
    public class Logger
    {
        private static string SettingsDirectory
        {
            get
            {
                return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "purevpn beta (rv)");
            }
        }
        ...
        public static void Log(string LogMessage, LogTag TAG = LogTag.INFO)
        {
            try
            {
                try
                {
                    ...
                    string messageToLog = string.Format("{0}\t|\t{1}\t|\t{2}\t{3}", DateTime.Now, "purevpn beta (rv)", TAG, LogMessage);
